...
// TODO: handle interstitial tabs chars
...
struct MM_Context{
  ...
  Range_i64 _range; // visual pos range of _line
  Vec2_f32  _p0;
};
...
function Vec2_f32 MM_get_xy(Application_Links *app, i64 pos){
  if (!range_contains(g_mm_ctx._range, pos)){
    ...
    Vec2_f32 p          = buffer_relative_box_of_pos(app, g_mm_ctx.buffer, g_mm_ctx.width, g_mm_ctx.face, line, pos).p0;
    g_mm_ctx._range.min = buffer_pos_at_relative_xy (app, g_mm_ctx.buffer, g_mm_ctx.width, g_mm_ctx.face, line, V2f32(0.f,     p.y));
    g_mm_ctx._range.max = buffer_pos_at_relative_xy (app, g_mm_ctx.buffer, g_mm_ctx.width, g_mm_ctx.face, line, V2f32(max_f32, p.y));
    g_mm_ctx._rel.x  =    buffer_relative_box_of_pos(app, g_mm_ctx.buffer, g_mm_ctx.width, g_mm_ctx.face, line, g_mm_ctx._range.min).x0;
    g_mm_ctx._rel.y +=    buffer_line_y_difference  (app, g_mm_ctx.buffer, g_mm_ctx.width, g_mm_ctx.face, line, g_mm_ctx._line);
    g_mm_ctx._line = line;
    g_mm_ctx._p0 = g_mm_ctx.region.p0 + hadamard(g_mm_ctx.s_p0, g_mm_ctx._rel + V2f32(0, p.y));
  }
  return g_mm_ctx._p0 + g_mm_ctx.s_str.x * V2f32(f32(pos - g_mm_ctx._range.min), 0);
}

function Rect_f32 MM_get_line(Application_Links *app, i64 line){
  Range_i64 range = get_line_pos_range(app, g_mm_ctx.buffer, line);
  Range_f32 y = If32(MM_get_xy(app, range.min).y,
                     MM_get_xy(app, range.max).y + g_mm_ctx.s_str.y);
  return Rf32(rect_range_x(g_mm_ctx.outer), y);
}
...
function Text_Layout_ID MM_begin(Application_Links *app, Arena *arena, View_ID view, Face_ID face, Buffer_ID buffer, Token_Array tokens, Rect_f32 region, Range_i64 visible_range, ARGB_Color cl_nest){
  ...
  g_mm_ctx.width  = rect_width(region);
  ...
  Rect_f32 r = rect_union(MM_get_line(app, la),
                          MM_get_line(app, lb));
  Rect_f32 ra = MM_get_line(app, get_line_number_from_pos(app, buffer, view_get_cursor_pos(app, view)));
  Rect_f32 rb = MM_get_line(app, get_line_number_from_pos(app, buffer, view_get_mark_pos(app, view)));
