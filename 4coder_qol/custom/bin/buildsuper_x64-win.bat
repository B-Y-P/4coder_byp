...
echo -- [%time%]: Building
REM echo -- [%time%]: Metadata build pass
...
REM call cl %opts% /O2 "%custom_root%\4coder_metadata_generator.cpp" /Femetadata_generator || goto fail

REM echo.
REM echo -- [%time%]: Running metadata_generator
metadata_generator -R "%custom_root%" "%cd%\%preproc_file%"

REM echo.
REM echo -- [%time%]: Building custom layer
