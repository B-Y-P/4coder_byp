...
#define ID_METADATA_OUT "generated/managed_id_metadata.h"
...
struct Meta_Command_Entry_Arrays{
  ...
  *opt_pos_out = token.pos;
  ...
  static b32 require_token(Reader *reader, Token_Cpp_Kind kind, i64 *opt_pos_out = 0){
  b32 success = false;
  ...
  *opt_pos_out = token.pos;
  ...
  static b32
  accept_comma(Reader *reader, i64 *opt_pos_out = 0){
  b32 success = false;
  ...
  *opt_pos_out = token.pos;
  ...
  static b32
  require_comma(Reader *reader, i64 *opt_pos_out = 0){
  if (accept_comma(reader, opt_pos_out)){ return true; }
  error(reader, peek_pos(reader), (u8*)"expected to find ','");
  return false;
  }
  ...
  *opt_pos_out = token.pos;
  ...
  String_Const_u8 mc_kind = string_u8_litexpr("0");
  ...
  if (accept_comma(reader)){
  if (!extract_integer(reader, &mc_kind)){
  return(false);
  }
  }
  ...
  for (i32 i = 0; i < entry_count; ++i){
  Meta_Command_Entry *entry = entries[i];
  fprintf(cmd_out, "void %.*s(struct Application_Links *app);\n", string_expand(entry->name));
  }
  ...
  node != 0;
  node = node->next){
  fprintf(id_out, "global Managed_ID %.*s;\n", string_expand(node->id_name));
  }
